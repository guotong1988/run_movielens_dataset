...
class RecModel(object):
    ...
    def __init__(self,
        ...
        with tf.variable_scope('MLP', reuse=tf.AUTO_REUSE):
            last_layer1 = ensemble_layer(feature_list)

        bert_config = modeling.BertConfig.from_json_file("bert/config.json")
        bert_input_emb = tf.stack(feature_list, 1)
        tmp = tf.sign(self.input_ids_batch)
        bert_mask_ids = tf.cast(tmp, tf.float32)
        bert_model = modeling.BertModel(
            config=bert_config,
            is_training=is_training,
            input_ids=bert_input_emb,
            input_mask=bert_mask_ids,
            scope="bert")
        last_layer2 = tf.reduce_mean(bert_model.get_sequence_output(), 1) + last_layer1

        self.logits = tf.layers.dense(inputs=last_layer2,
                                      units=1,
